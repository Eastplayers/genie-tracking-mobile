...
- **UserDefaults**: iOS persistent storage mechanism for user preferences

## Requirements

### Requirement 1

**User Story:** As a developer testing the iOS SDK, I want to configure the API key, brand ID, and API URL directly in the app UI, so that I don't need to manually edit configuration files or set environment variables.
...
1. WHEN the user successfully initializes the tracker with configuration values THEN the system SHALL persist the API key, brand ID, selected environment, and user ID to UserDefaults
...
4. WHEN configuration is changed THEN the system SHALL persist the new values to UserDefaults
